...
pub const RateLimiter = struct {
    ...
    pub fn process(self: *Self, ctx: *RequestContext) !Middleware.NextAction {
        ...
        if (self.isRateLimited(identifier, now)) {
            try self.addRateLimitHeaders(ctx, identifier, now);
            return .respond; // 阻止请求
        }
    ...
    /// 添加 X-RateLimit-* 响应头
    fn addRateLimitHeaders(self: *const Self, ctx: *RequestContext, identifier: []const u8, now: i128) !void {
        // 注意：这里需要访问 Response 对象
        // 由于中间件接口限制，这里只是框架代码
        // 实际实现需要在路由分发时处理
        _ = self;
        _ = ctx;
